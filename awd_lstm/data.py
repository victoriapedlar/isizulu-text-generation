        ...
        if use_bpe:
            ...
            with open(os.path.join(path, "train.txt"), "r", encoding="utf-8") as f:
                ...
                for index, id in enumerate(enc.ids):
                    ids[index] = id
                self.train = ids
                self.dictionary.avg_characters_per_token["train"] = len(text) / len(
                    enc.ids
                )
            ...
            with open(os.path.join(path, "valid.txt"), "r", encoding="utf-8") as f:
                ...
                for index, id in enumerate(enc.ids):
                    ids[index] = id
                self.valid = ids
                self.dictionary.avg_characters_per_token["valid"] = len(text) / len(
                    enc.ids
                )
            ...
            self.dictionary.idx2word = [
                tokenizer.id_to_token(x) for x in range(tokenizer.get_vocab_size())
            ]
