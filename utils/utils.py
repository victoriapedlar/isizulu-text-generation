...
def clean_sentences(sentences, min_length=1, illegal_substrings=[], lines_to_remove=0):
    ...
    sentences = [
        sentence[re.search(r"[a-zA-Z]", sentence).start() :]
        for sentence in sentences
        if re.search(r"[a-zA-Z]", sentence) is not None
    ]
    ...
    sentences = [
        re.sub(r"[A-Z -]+", lambda m: m.group().capitalize(), sentence, count=1)
        for sentence in sentences
    ]
    ...
    sentences = [
        sentence for sentence in sentences if d[sentence] < max_repetitions_allowed
    ]
