...
class LayerSwitchingGPT2Config(PretrainedConfig):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.inverse_language_specific_transformation = (
...
class LayerSwitchingBlock(nn.Module):
    ...
    def __init__(self, n_ctx, config, scale=False, n_languages=1):
        ...
        self.attn = nn.ModuleList(
            [Attention(hidden_size, n_ctx, config, scale) for _ in range(n_languages)]
        )
        # -------------------------------END CUSTOM CODE-----------------------------------
...
class LayerSwitchingGPT2Model(GPT2PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if config.language_specific_input_embeds:
            ...
            self.wte = nn.ModuleList([nn.Embedding(config.vocab_size, config.n_embd)])
        ...
        for i in range(config.n_layer):
            ...
            if i < config.n_language_specific_attention_layers:
                self.h.append(
                    LayerSwitchingBlock(
                        config.n_ctx, config, scale=True, n_languages=config.n_languages
                    )
                )
            else:
                self.h.append(
                    LayerSwitchingBlock(config.n_ctx, config, scale=True, n_languages=1)
                )
            # ------------------------------END CUSTOM CODE-----------------------------------
    ...
    def _init_weights(self, module):
        ...
        """
        if isinstance(
            module, (nn.Linear, nn.Embedding, Conv1D, SemanticEmbedding)
        ):  # CUSTOM CODE
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.config.language_specific_transformation:
            inputs_embeds = torch.tanh(
                self.language_specific_transformations[language](inputs_embeds)
            )
        ...
        for i, (block, layer_past) in enumerate(zip(self.h, past_key_values)):
            ...
            outputs = block(
                ...
                output_attentions=output_attentions,
                language=language
                if i < self.config.n_language_specific_attention_layers
                else 0,  # CUSTOM CODE
            )
        ...
        # ------------------------------START CUSTOM CODE----------------------------------
        if self.config.inverse_language_specific_transformation:
            hidden_states = torch.tanh(
                self.inverse_language_specific_transformations[language](hidden_states)
            )
        # -------------------------------END CUSTOM CODE-----------------------------------
...
class GPT2LayerSwitchingLMHeadModel(GPT2PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        # ------------------------------START CUSTOM CODE----------------------------------
        if (
            self.config.language_specific_input_embeds
            != self.config.language_specific_prediction_heads
        ):
            assert not config.tie_word_embeddings

        assert (
            config.n_languages is not None and config.n_languages > 0
        ), "n_languages must be positive"
    ...
    def tie_weights(self):
        ...
        if self.config.tie_word_embeddings:
            # ------------------------------START CUSTOM CODE----------------------------------
            for input_embeddings, output_embeddings in zip(
