...
def top_k_top_p_filtering(
    logits, top_k=0, top_p=0.0, filter_value=-float("Inf"), gen_func=torch.softmax
):
    ...
    if top_k > 0:
        for i in range(logits.size(0)):
            indices_to_remove = (
                logits[i] < torch.topk(logits[i], top_k)[0][..., -1, None]
            )
...
def sample_sequence(
    model, prefix_batch, prefix_length, continuation_length, top_k, top_p
):
    continuation_logits = []
...
def ul_seq(model, batch, args):
    ...
    completions, continuation_logits = sample_sequence(
        model, batch, 50, 100, args.top_k, args.top_p
    )
    ...
    one_minus_probs = torch.clamp((1.0 - pred_lprobs.exp()), min=1e-20).view(
...
def repeat_at_1(args, predictions, targets, context_length, topk=0, topp=0.0):
    with torch.no_grad():
        ...
        prev_targets = (
            ...
            .tril()
            .masked_fill_(
                torch.ones_like(targets.expand(T, T)).byte().triu().bool(), -1
            )
        )

        # each row t is [-1, ..., -1, y_{t-k-1}, ..., y_{t-1}, -1, ..., -1] where k is context length
        prev_targets = prev_targets.masked_fill_(
            torch.ones_like(targets.expand(T, T))
            .byte()
            .tril(-(context_length + 1))
            .bool(),
            -1,
        )
        ...
        total_human_repeat_at_1 = (
            (prev_targets == targets.view(T, 1)).sum(1).gt(0).sum()
        )

    return total_repeat_at_1.item() / float(
        targets.size(1)
    ), total_wrong_repeat_at_1.item() / float(targets.size(1))
    ...
    if args.local_rank in [-1, 0]:
        if args.loss == "entmax":
            tb_writer = SummaryWriter(
                filename_suffix="_entmax_" + str(args.entmax_alpha)
            )
        elif args.top_p > 0:
        ...
        with torch.no_grad():
            outputs = (
                ...
                if args.mlm
                    ...
                    batch, labels=batch, unlikelihood=False, unlikelihood_seq=False
                )
            )
            ...
            if args.temp != 0:
                ...
                shift_logits = top_k_top_p_filtering(
                    shift_logits, top_p=args.top_p, top_k=args.top_k, gen_func=gen_func
                )
            ...
            repeat, wrong_repeat = repeat_at_1(
                args, pred, shift_labels, 16, topk=args.top_k, topp=top_p
            )
            repeat_16.append(repeat)
            wrong_repeat_16.append(wrong_repeat)
            repeat, wrong_repeat = repeat_at_1(
                args, pred, shift_labels, 32, topk=args.top_k, topp=top_p
            )
            repeat_32.append(repeat)
            wrong_repeat_32.append(wrong_repeat)
            repeat, wrong_repeat = repeat_at_1(
                args, pred, shift_labels, 128, topk=args.top_k, topp=top_p
            )
            repeat_128.append(repeat)
            wrong_repeat_128.append(wrong_repeat)
            repeat, wrong_repeat = repeat_at_1(
                args, pred, shift_labels, 512, topk=args.top_k, topp=top_p
            )
...
def main():
    ...
    tokenizer = GPT2TokenizerFast(
        vocab_file="./tokenizers/ByteLevelBPETokenizer/vocab.json",
        merges_file="./tokenizers/ByteLevelBPETokenizer/merges.txt",
        tokenizer=tokenizer,
    )
    # ------------------END CUSTOM CODE--------------------#
    ...
    generic_entmax_loss = partial(
        EntmaxBisectLoss, alpha=args.entmax_alpha, n_iter=args.entmax_bisect_iter
    )
    ...
    generic_entmax = partial(
        entmax_bisect, alpha=args.entmax_alpha, n_iter=args.entmax_bisect_iter
    )
